/*
API Documentation

Source of truth and network automation platform

API version: 1.3.7 (1.2)
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// PowerPanelRackGroup struct for PowerPanelRackGroup
type PowerPanelRackGroup struct {
	Id string `json:"id"`
	Url string `json:"url"`
	Name string `json:"name"`
	Slug *string `json:"slug,omitempty"`
	RackCount int32 `json:"rack_count"`
	Depth int32 `json:"_depth"`
	// Human friendly display value
	Display string `json:"display"`
}

// NewPowerPanelRackGroup instantiates a new PowerPanelRackGroup object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPowerPanelRackGroup(id string, url string, name string, rackCount int32, depth int32, display string) *PowerPanelRackGroup {
	this := PowerPanelRackGroup{}
	this.Id = id
	this.Url = url
	this.Name = name
	this.RackCount = rackCount
	this.Depth = depth
	this.Display = display
	return &this
}

// NewPowerPanelRackGroupWithDefaults instantiates a new PowerPanelRackGroup object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPowerPanelRackGroupWithDefaults() *PowerPanelRackGroup {
	this := PowerPanelRackGroup{}
	return &this
}

// GetId returns the Id field value
func (o *PowerPanelRackGroup) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *PowerPanelRackGroup) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *PowerPanelRackGroup) SetId(v string) {
	o.Id = v
}

// GetUrl returns the Url field value
func (o *PowerPanelRackGroup) GetUrl() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Url
}

// GetUrlOk returns a tuple with the Url field value
// and a boolean to check if the value has been set.
func (o *PowerPanelRackGroup) GetUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Url, true
}

// SetUrl sets field value
func (o *PowerPanelRackGroup) SetUrl(v string) {
	o.Url = v
}

// GetName returns the Name field value
func (o *PowerPanelRackGroup) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *PowerPanelRackGroup) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *PowerPanelRackGroup) SetName(v string) {
	o.Name = v
}

// GetSlug returns the Slug field value if set, zero value otherwise.
func (o *PowerPanelRackGroup) GetSlug() string {
	if o == nil || o.Slug == nil {
		var ret string
		return ret
	}
	return *o.Slug
}

// GetSlugOk returns a tuple with the Slug field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PowerPanelRackGroup) GetSlugOk() (*string, bool) {
	if o == nil || o.Slug == nil {
		return nil, false
	}
	return o.Slug, true
}

// HasSlug returns a boolean if a field has been set.
func (o *PowerPanelRackGroup) HasSlug() bool {
	if o != nil && o.Slug != nil {
		return true
	}

	return false
}

// SetSlug gets a reference to the given string and assigns it to the Slug field.
func (o *PowerPanelRackGroup) SetSlug(v string) {
	o.Slug = &v
}

// GetRackCount returns the RackCount field value
func (o *PowerPanelRackGroup) GetRackCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.RackCount
}

// GetRackCountOk returns a tuple with the RackCount field value
// and a boolean to check if the value has been set.
func (o *PowerPanelRackGroup) GetRackCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.RackCount, true
}

// SetRackCount sets field value
func (o *PowerPanelRackGroup) SetRackCount(v int32) {
	o.RackCount = v
}

// GetDepth returns the Depth field value
func (o *PowerPanelRackGroup) GetDepth() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Depth
}

// GetDepthOk returns a tuple with the Depth field value
// and a boolean to check if the value has been set.
func (o *PowerPanelRackGroup) GetDepthOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Depth, true
}

// SetDepth sets field value
func (o *PowerPanelRackGroup) SetDepth(v int32) {
	o.Depth = v
}

// GetDisplay returns the Display field value
func (o *PowerPanelRackGroup) GetDisplay() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Display
}

// GetDisplayOk returns a tuple with the Display field value
// and a boolean to check if the value has been set.
func (o *PowerPanelRackGroup) GetDisplayOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Display, true
}

// SetDisplay sets field value
func (o *PowerPanelRackGroup) SetDisplay(v string) {
	o.Display = v
}

func (o PowerPanelRackGroup) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["id"] = o.Id
	}
	if true {
		toSerialize["url"] = o.Url
	}
	if true {
		toSerialize["name"] = o.Name
	}
	if o.Slug != nil {
		toSerialize["slug"] = o.Slug
	}
	if true {
		toSerialize["rack_count"] = o.RackCount
	}
	if true {
		toSerialize["_depth"] = o.Depth
	}
	if true {
		toSerialize["display"] = o.Display
	}
	return json.Marshal(toSerialize)
}

type NullablePowerPanelRackGroup struct {
	value *PowerPanelRackGroup
	isSet bool
}

func (v NullablePowerPanelRackGroup) Get() *PowerPanelRackGroup {
	return v.value
}

func (v *NullablePowerPanelRackGroup) Set(val *PowerPanelRackGroup) {
	v.value = val
	v.isSet = true
}

func (v NullablePowerPanelRackGroup) IsSet() bool {
	return v.isSet
}

func (v *NullablePowerPanelRackGroup) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePowerPanelRackGroup(val *PowerPanelRackGroup) *NullablePowerPanelRackGroup {
	return &NullablePowerPanelRackGroup{value: val, isSet: true}
}

func (v NullablePowerPanelRackGroup) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePowerPanelRackGroup) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



/*
API Documentation

Source of truth and network automation platform

API version: 1.3.10b1 (1.2)
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// WritableSecretsGroupAssociation Serializer for `SecretsGroupAssociation` objects.
type WritableSecretsGroupAssociation struct {
	Id string `json:"id"`
	Url string `json:"url"`
	Group string `json:"group"`
	AccessType AccessTypeEnum `json:"access_type"`
	SecretType SecretTypeEnum `json:"secret_type"`
	Secret string `json:"secret"`
	// Human friendly display value
	Display string `json:"display"`
}

// NewWritableSecretsGroupAssociation instantiates a new WritableSecretsGroupAssociation object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWritableSecretsGroupAssociation(id string, url string, group string, accessType AccessTypeEnum, secretType SecretTypeEnum, secret string, display string) *WritableSecretsGroupAssociation {
	this := WritableSecretsGroupAssociation{}
	this.Id = id
	this.Url = url
	this.Group = group
	this.AccessType = accessType
	this.SecretType = secretType
	this.Secret = secret
	this.Display = display
	return &this
}

// NewWritableSecretsGroupAssociationWithDefaults instantiates a new WritableSecretsGroupAssociation object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWritableSecretsGroupAssociationWithDefaults() *WritableSecretsGroupAssociation {
	this := WritableSecretsGroupAssociation{}
	return &this
}

// GetId returns the Id field value
func (o *WritableSecretsGroupAssociation) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *WritableSecretsGroupAssociation) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *WritableSecretsGroupAssociation) SetId(v string) {
	o.Id = v
}

// GetUrl returns the Url field value
func (o *WritableSecretsGroupAssociation) GetUrl() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Url
}

// GetUrlOk returns a tuple with the Url field value
// and a boolean to check if the value has been set.
func (o *WritableSecretsGroupAssociation) GetUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Url, true
}

// SetUrl sets field value
func (o *WritableSecretsGroupAssociation) SetUrl(v string) {
	o.Url = v
}

// GetGroup returns the Group field value
func (o *WritableSecretsGroupAssociation) GetGroup() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Group
}

// GetGroupOk returns a tuple with the Group field value
// and a boolean to check if the value has been set.
func (o *WritableSecretsGroupAssociation) GetGroupOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Group, true
}

// SetGroup sets field value
func (o *WritableSecretsGroupAssociation) SetGroup(v string) {
	o.Group = v
}

// GetAccessType returns the AccessType field value
func (o *WritableSecretsGroupAssociation) GetAccessType() AccessTypeEnum {
	if o == nil {
		var ret AccessTypeEnum
		return ret
	}

	return o.AccessType
}

// GetAccessTypeOk returns a tuple with the AccessType field value
// and a boolean to check if the value has been set.
func (o *WritableSecretsGroupAssociation) GetAccessTypeOk() (*AccessTypeEnum, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AccessType, true
}

// SetAccessType sets field value
func (o *WritableSecretsGroupAssociation) SetAccessType(v AccessTypeEnum) {
	o.AccessType = v
}

// GetSecretType returns the SecretType field value
func (o *WritableSecretsGroupAssociation) GetSecretType() SecretTypeEnum {
	if o == nil {
		var ret SecretTypeEnum
		return ret
	}

	return o.SecretType
}

// GetSecretTypeOk returns a tuple with the SecretType field value
// and a boolean to check if the value has been set.
func (o *WritableSecretsGroupAssociation) GetSecretTypeOk() (*SecretTypeEnum, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SecretType, true
}

// SetSecretType sets field value
func (o *WritableSecretsGroupAssociation) SetSecretType(v SecretTypeEnum) {
	o.SecretType = v
}

// GetSecret returns the Secret field value
func (o *WritableSecretsGroupAssociation) GetSecret() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Secret
}

// GetSecretOk returns a tuple with the Secret field value
// and a boolean to check if the value has been set.
func (o *WritableSecretsGroupAssociation) GetSecretOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Secret, true
}

// SetSecret sets field value
func (o *WritableSecretsGroupAssociation) SetSecret(v string) {
	o.Secret = v
}

// GetDisplay returns the Display field value
func (o *WritableSecretsGroupAssociation) GetDisplay() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Display
}

// GetDisplayOk returns a tuple with the Display field value
// and a boolean to check if the value has been set.
func (o *WritableSecretsGroupAssociation) GetDisplayOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Display, true
}

// SetDisplay sets field value
func (o *WritableSecretsGroupAssociation) SetDisplay(v string) {
	o.Display = v
}

func (o WritableSecretsGroupAssociation) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["id"] = o.Id
	}
	if true {
		toSerialize["url"] = o.Url
	}
	if true {
		toSerialize["group"] = o.Group
	}
	if true {
		toSerialize["access_type"] = o.AccessType
	}
	if true {
		toSerialize["secret_type"] = o.SecretType
	}
	if true {
		toSerialize["secret"] = o.Secret
	}
	if true {
		toSerialize["display"] = o.Display
	}
	return json.Marshal(toSerialize)
}

type NullableWritableSecretsGroupAssociation struct {
	value *WritableSecretsGroupAssociation
	isSet bool
}

func (v NullableWritableSecretsGroupAssociation) Get() *WritableSecretsGroupAssociation {
	return v.value
}

func (v *NullableWritableSecretsGroupAssociation) Set(val *WritableSecretsGroupAssociation) {
	v.value = val
	v.isSet = true
}

func (v NullableWritableSecretsGroupAssociation) IsSet() bool {
	return v.isSet
}

func (v *NullableWritableSecretsGroupAssociation) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWritableSecretsGroupAssociation(val *WritableSecretsGroupAssociation) *NullableWritableSecretsGroupAssociation {
	return &NullableWritableSecretsGroupAssociation{value: val, isSet: true}
}

func (v NullableWritableSecretsGroupAssociation) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWritableSecretsGroupAssociation) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


